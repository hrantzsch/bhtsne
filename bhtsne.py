...
BH_TSNE_BIN_PATH = path_join(dirname(__file__), 'windows', 'bh_tsne.exe')\
...
def _argparse():
    ...
    argparse.add_argument('-o', '--output', type=FileType('w'),
                          default=stdout)
    argparse.add_argument('-c', '--classes', type=int, default=10)
...
if __name__ == '__main__':
    ...
    # MNIST
    # data = pickle.load(open(argp.input, 'rb'))
    ...
    for result in bh_tsne(data, no_dims=argp.no_dims,
                          ...
                          randseed=argp.randseed, verbose=argp.verbose):
        fmt = ''
